...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Reservas e Cancelamentos</title>
    <style>
...
<body>
    <div class="container">
        ...
        <h2>Detalhamento de Reservas e Cancelamentos</h2>
        <table class="details-table">
            ...
            <tbody>
                @foreach($dados['labels'] as $index => $label)
                    <tr>
                        <td>{{ $label }}</td>
                        <td>{{ $dados['data_criadas'][$index] }}</td>
                        <td>{{ $dados['data_canceladas'][$index] }}</td>
                        <td>{{ $dados['data_apagadas'][$index] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals">
            <table class="totals-table">
                <tr>
                    <th>Total Reservas Criadas</th>
                    <td>{{ $dados['total_criadas'] }}</td>
                </tr>
                <tr>
                    <th>Total Reservas Canceladas</th>
                    <td>{{ $dados['total_canceladas'] }}</td>
                </tr>
                <tr class="grand-total">
                    <th>Total Reservas Apagadas</th>
                    <td>{{ $dados['total_apagadas'] }}</td>
                </tr>
            </table>
        </div>
