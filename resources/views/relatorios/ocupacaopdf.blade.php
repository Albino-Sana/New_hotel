...
<body>
    <div class="container">
        ...
        <h2>Detalhamento de Ocupação</h2>
        ...
        <table class="details-table">
            ...
            <tbody>
                @foreach($dados['checkins'] as $checkin)
                    <tr>
                         <td>{{ $checkin->reserva->cliente_nome ?? 'Excluido' }}</td> <!-- Ajuste o nome da coluna conforme necessário -->
                        <td>{{ \Carbon\Carbon::parse($checkin->data_entrada)->format('d/m/Y H:i') }}</td>
                        <td>{{ $checkin->data_saida ? \Carbon\Carbon::parse($checkin->data_saida)->format('d/m/Y H:i') : 'Em aberto' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals">
            <table class="totals-table">
                <tr class="grand-total">
                    <th>Total de Quartos Ocupados</th>
                    <td>{{ $dados['total_ocupacao'] }}</td>
                </tr>
            </table>
        </div>
